...
echo "********* You get an error prompt_continue_ccloud_demo': not a valid identifier then make sure you are in bash shell *********"

DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" >/dev/null && pwd )"
VALIDATE_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" >/dev/null && pwd )"
echo "Starting Step 1 - Login to Confluent Cloud"
...
echo "Starting Step 2 - Create a stack in Confluent Cloud" 
echo
...
echo "Starting Step 3 - Set a Cluster in Confluent Cloud"
...
echo "Starting Step 5 - Create ACL and Set to Service Account in Confluend Cloud"
